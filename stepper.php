...
if ($isUserLoggedIn && isset($conn) && $conn instanceof mysqli) {
    // 1. Check for active ticket (Menunggu, Dipanggil, Dilayani)
    ...
    // 2. Check for latest completed ticket for history / review reference
...
<script>
let lastStepperTicketStatus = '<?php echo htmlspecialchars($latestTicket['status'] ?? ''); ?>';
let lastStepperTicketId = <?php echo (int)($latestTicket['id'] ?? 0); ?>;
...
if (sessionStorage.getItem('stepper_user_reset_new') === '1') {
  const hasActive = <?php echo $hasActiveTicket ? 'true' : 'false'; ?>;
  if (!hasActive) {
